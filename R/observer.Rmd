...
if (!dir.exists("./sim-fit")) {
  dir.create("./sim-fit")
}

if (!dir.exists("./observer-fit")) {
  dir.create("./observer-fit")
}

if (!dir.exists("./outlier-fit")) {
...
matplot(locs, makePCA(matrix(unlist(field$Z_list[(reps / 2 + 1):reps]), 
        ...
        main = "", ylim =c(min(noisy_plot), max(noisy_plot)), lty=1, 
        col=adjustcolor(1:4, alpha.f=0.6))
mtext("(d)",side=3,line=-1.9,adj=c(0.1),cex=1,col="black")
```

```{r simPlot2, echo=FALSE, include=FALSE, fig.height=6, fig.width=12, eval=TRUE}
library(RColorBrewer)
c25 <- c("dodgerblue2","#E31A1C", # red
         "green4",
         "#6A3D9A", # purple
...
matplot(locs, matrix(unlist(Z_list_pca), m, reps/2), type='n',
        ...
        ylim =c(min(unlist(Z_list_noisy)), max(unlist(Z_list_noisy))),
        xlab="", xaxt="n", yaxt="n", ylab="", add=TRUE)
# for (i in 1:(reps/2)) {
for (i in 1:10) {
  matplot(locs, Z_list_pca[[i]], type='l', add=TRUE, lwd=2,
          col=adjustcolor(c25[i], alpha.f=0.5),
          xlab="", xaxt="n", yaxt="n", ylab="")
}
# mtext("(b)",side=3,line=-1.9,adj=c(0.1),cex=1,col="black")
...
matplot(locs, makePCA(matrix(unlist(field$Z_list[(reps / 2 + 1):reps]), 
                             ncol = reps / 2, byrow = FALSE))$X_pca[, 1:4], 
        type='l', yaxt="n", ylab="", xlab="Location",
        main = "", ylim =c(min(noisy_plot), max(noisy_plot)), lty=1, lwd=2,
        col=adjustcolor(1:4, alpha.f=0.6), add=TRUE)
# mtext("(d)",side=3,line=-1.9,adj=c(0.1),cex=1,col="black")
```

\begin{figure}
\centering\includegraphics[width=0.95\linewidth]{observer_files/figure-latex/simPlot-1.pdf}
...
if (file.exists("./sim-fit/pcassvs.RData")) {
  ...
  samps <- foreach(i=1:n_chains, .inorder = FALSE, 
                   ...
                   .multicombine = TRUE) %dopar% {
                     model.jags <- jags.model("./models/PCASSVS.txt", data=dat,
                                              inits=jinits,
  ...
  save(y_tilde_mean_pca_ssvs, y_tilde_sd_pca_ssvs, loo_pca_ssvs, 
       crps_pca_ssvs, file="./sim-fit/pcassvs.RData")
}
```
...
samps <- foreach(i=1:n_chains, .inorder = FALSE, 
                 .packages = c("rjags", "random"), .combine = "mcmc.combine", 
                 .multicombine = TRUE) %dopar% {
                   model.jags <- jags.model("./models/PCALasso.txt", 
...
rm(samps, summaries, betas, s_invs)
  save(y_tilde_mean_pca_lasso, y_tilde_sd_pca_lasso, loo_pca_lasso, 
       crps_pca_lasso, file="./sim-fit/pcalasso.RData")
}
```
...
samps <- foreach(i=1:n_chains, .inorder = FALSE, 
                 .packages = c("rjags", "random"), .combine = "mcmc.combine", 
                 .multicombine = TRUE) %dopar% {
                   model.jags <- jags.model("./models/tPCASSVS.txt",
                                            data=dat, inits=jinits, n.chain=1, n.adapt=n_adapt)
                   update(model.jags, n_burn)
                   result <- coda.samples(model.jags, params, n_iter)
                   return(result)
...
rm(samps, nus, v_invs, s_invs, betas, log_like, summaries)
  save(y_tilde_mean_tpca_ssvs, y_tilde_sd_tpca_ssvs, loo_tpca_ssvs, 
       crps_tpca_ssvs, file="./sim-fit/tpcassvs.RData")
}
```
...
samps <- foreach(i=1:n_chains, .inorder = FALSE, 
                 .packages = c("rjags", "random"), .combine = "mcmc.combine", 
                 .multicombine = TRUE) %dopar% {
                   model.jags <- jags.model("./models/tPCALasso.txt", 
                                            data=dat, inits=jinits, n.chain=1, n.adapt=n_adapt)
                   update(model.jags, n_burn)
                   result <- coda.samples(model.jags, params, n_iter)
                   return(result)
...
rm(samps, nus, v_invs, s_invs, betas, log_like, summaries)
  save(y_tilde_mean_tpca_lasso, y_tilde_sd_tpca_lasso, loo_tpca_lasso, 
       crps_tpca_lasso, file="./sim-fit/tpcalasso.RData")
}
```
...
samps <- foreach(i=1:n_chains, .inorder = FALSE, 
                 .packages = c("rjags", "random"), .combine = "mcmc.combine", 
                 .multicombine = TRUE) %dopar% {
                   model.jags <- jags.model("./models/pPCASSVS.txt", data=dat,
                                            inits=jinits,
                                            n.chain=1, n.adapt=n_adapt)
                   update(model.jags, n_burn)
                   result <- coda.samples(model.jags, params, n_iter)
...
rm(samps, summaries, betas, s2s, tau2s)
  save(y_tilde_mean_ppca_ssvs, y_tilde_sd_ppca_ssvs, loo_ppca_ssvs, 
       crps_ppca_ssvs, file="./sim-fit/ppcassvs.RData")
}
```
...
samps <- foreach(i=1:n_chains, .inorder = FALSE, 
                 .packages = c("rjags", "random"), .combine = "mcmc.combine", 
                 .multicombine = TRUE) %dopar% {
                   model.jags <- jags.model("./models/pPCALasso.txt",
                                            data=dat, inits=jinits, n.chain=1, n.adapt=n_adapt)
                   update(model.jags, n_burn)
                   result <- coda.samples(model.jags, params, n_iter)
                   return(result)
...
rm(samps, summaries, betas, s2s, tau2s)
  save(y_tilde_mean_ppca_lasso, y_tilde_sd_ppca_lasso, loo_ppca_lasso, 
       crps_ppca_lasso, file="./sim-fit/ppcalasso.RData")
}
```
...
samps <- foreach(i=1:n_chains, .inorder = FALSE, 
                 .packages = c("rjags", "random"), .combine = "mcmc.combine", 
                 .multicombine = TRUE) %dopar% {
                   model.jags <- jags.model("./models/tpPCASSVS.txt",
                                            data=dat, inits=jinits, n.chain=1, n.adapt=n_adapt)
                   update(model.jags, n_burn)
                   result <- coda.samples(model.jags, params, n_iter)
                   return(result)
...
rm(samps, nus, v_invs, s2s, tau2s, betas, log_like, summaries)
  save(y_tilde_mean_tppca_ssvs, y_tilde_sd_tppca_ssvs, loo_tppca_ssvs, 
       crps_tppca_ssvs, file="./sim-fit/tppcassvs.RData")
}
```

```{r tpPCALassoSim, echo=FALSE, eval=TRUE}
if (file.exists("./sim-fit/tppcalasso.RData")) {
...
samps <- foreach(i=1:n_chains, .inorder = FALSE, 
                 .packages = c("rjags", "random"), .combine = "mcmc.combine", 
                 .multicombine = TRUE) %dopar% {
                   model.jags <- jags.model("./models/tpPCALasso.txt", 
                                            data=dat, inits=jinits, n.chain=1, n.adapt=n_adapt)
                   update(model.jags, n_burn)
                   result <- coda.samples(model.jags, params, n_iter)
                   return(result)
...
rm(samps, nus, v_invs, s2s, tau2s, betas, log_like, summaries)
  save(y_tilde_mean_tppca_lasso, y_tilde_sd_tppca_lasso, loo_tppca_lasso, 
       crps_tppca_lasso, file="./sim-fit/tppcalasso.RData")
}
```
...
c25 <- c("dodgerblue2","#E31A1C", # red
         ...
         "darkturquoise","green1","yellow4","yellow3",
         "darkorange4","brown")
plot(NA, type='n', xlim=c(-2, 4), ylim=c(-2, 4), xlab="Predicted", 
     ylab="Observed", main = 'Robust PCR LASSO Reconstruction')
for (i in 1:dim(Z_hist)[2]) {
  points(Z_hist[, i] ~ y_tilde_mean_tpca_lasso[, i], 
         col=adjustcolor(c25[i], alpha.f=0.25))
}
abline(0, 1)
plot(NA, type='n', xlim=c(-2, 4), ylim=c(-2, 4), xlab="Predicted", 
     ylab="Observed", main = 'Robust pPCR LASSO Reconstruction')

for (i in 1:dim(Z_hist)[2]) {
  points(Z_hist[, i] ~ y_tilde_mean_tppca_lasso[, i],
         col=adjustcolor(c25[i], alpha.f=0.25))
}
abline(0, 1)
```
...
\begin{figure}
\centering
\includegraphics[width=0.95\linewidth]{observer_files/figure-latex/pPCAreconstruction-1.pdf}
...
\begin{figure}
\centering
\includegraphics[width=0.95\linewidth]{observer_files/figure-latex/pPCAreconstructionSurfaces-1.pdf}
...
```{r data-plot-jsm, include=FALSE, echo=FALSE, fig.height=6, fig.width=12, eval=TRUE}
library(RColorBrewer)
...
# par(mfrow=c(2, 4), mar=c(0.5, 0.5, 1.75, 0.5) + 0.1, oma=c(0, 0, 0.25, 2.5) + 0.1)
png("~/Google Drive/Meetings/2017/JSM/ASCMO/observer-data.png", 
    width=480*6, height=480*4, res=400)
layout(matrix(c(1, 1, 1, 2, 2, 2, 5,
                1, 1, 1, 2, 2, 2, 5,
                1, 1, 1, 2, 2, 2, 5,
                3, 3, 3, 4, 4, 4, 5,
                3, 3, 3, 4, 4, 4, 5,
                3, 3, 3, 4, 4, 4, 5), 6, 7, byrow=TRUE))
par(mar=c(3.5, 3.25, 1.75, 0.5) + 0.1, oma=c(0, 0, 0.25, 0.25) + 0.1)
for(i in subset[1:2]){
  # palette(topo.colors(64))
  idx <- rep(0, length = length((values(fort.raster[[i]])[ - na.rows][H.list[[i]]])))
...
##
##
png("~/Google Drive/Meetings/2017/JSM/ASCMO/prism-data.png", 
    width=480*6, height=480*4, res=400)
layout(matrix(c(1, 1, 1, 2, 2, 2, 5,
                1, 1, 1, 2, 2, 2, 5,
                1, 1, 1, 2, 2, 2, 5,
...
```{r pca-plots-jsm}
prism_pca <- prism
prism_pca_min <- rep(0, 4)
prism_pca_max <- rep(0, 4)

for(i in 1:4){
  values(prism_pca[[i]])[ - na.rows] <- X_PCA$X_pca[, i]
  prism_pca_min[i] <- min(X_PCA$X_pca[, i])
  prism_pca_max[i] <- max(X_PCA$X_pca[, i])
}

png("~/Google Drive/Meetings/2017/JSM/ASCMO/prism-pca.png", 
    width=480*6, height=480*4, res=400)
layout(matrix(c(1, 1, 1, 2, 2, 2, 5,
                1, 1, 1, 2, 2, 2, 5,
                1, 1, 1, 2, 2, 2, 5,
...
for(i in 1:4){
  if (i == 1) {
    image(prism_pca[[i]], main = paste(i, "st PC", sep=""), 
               col = cm.colors(64), zlim = c(prism_pca_min[i], prism_pca_max[i]),
               ylab = '', xlab='', yaxt="n", xaxt="n", cex.main=2.5)
    mtext("latitude", 2, line=2)
  } else if (i == 2) {
    image(prism_pca[[i]], main = paste(i, "nd PC", sep=""), 
               col = cm.colors(64), zlim = c(prism_pca_min[i], prism_pca_max[i]),
               ylab = '', xlab='', yaxt="n", xaxt="n", cex.main=2.5)
  } else if (i == 3) {
    image(prism_pca[[i]], main = paste(i, "rd PC", sep=""), 
               col = cm.colors(64), zlim = c(prism_pca_min[i], prism_pca_max[i]),
               ylab = '', xlab='', yaxt="n", xaxt="n", cex.main=2.5)
    mtext("latitude", 2, line=2)
    mtext("longitude", 1, line=2.25)
  } else if (i == 4) {
    image(prism_pca[[i]], main = paste(i, "th PC", sep=""), 
               col = cm.colors(64), zlim = c(prism_pca_min[i], prism_pca_max[i]),
               ylab = '', xlab='', yaxt="n", xaxt="n", cex.main=2.5)
    mtext("longitude", 1, line=2.25)
  }
...
image.scale(prism_pca[[i]], col=rev(cm.colors(64)),
            #col = topo.colors(64),
            zlim = c(min(prism_pca_min), max(prism_pca_max)), horiz=FALSE)
dev.off()
```

\begin{figure}
\centering
\centering\includegraphics[width=0.95\linewidth]{observer_files/figure-latex/data-plot-1.pdf}
...
if (file.exists("./observer-fit/pcassvs.RData")) {
  ...
  samps <- foreach(i=1:n_chains, .inorder = FALSE,
                   .packages = c("rjags", "random"), .combine = "mcmc.combine",
                   .multicombine = TRUE) %dopar% {
                     model.jags <- jags.model("./models/PCASSVS.txt",
                                              data=dat, inits=jinits, n.chain=1, n.adapt=n_adapt)
                     update(model.jags, n_burn)
                     result <- coda.samples(model.jags, params, n_iter)
                     return(result)
...
samps <- foreach(i=1:n_chains, .inorder = FALSE,
                 .packages = c("rjags", "random"), .combine = "mcmc.combine",
                 .multicombine = TRUE) %dopar% {
                   model.jags <- jags.model("./models/pPCASSVS.txt",
                                            data=dat, inits=jinits, n.chain=1, n.adapt=n_adapt)
                   update(model.jags, n_burn)
                   result <- coda.samples(model.jags, params, n_iter)
                   return(result)
...
\centering
\input{./loooutlier}
\caption{Fort historical reconstruction scores with outlier removed. Smaller values indicate better model performance}
\label{tab:fort}
\end{table}
...
\begin{figure}
\centering\includegraphics[width=0.95\linewidth]{observer_files/figure-latex/loo_fort-1.pdf}
...
\begin{figure}
\centering\includegraphics[width=0.95\linewidth]{observer_files/figure-latex/loo_fort2-1.pdf}
\caption{Plot of Fort data LOO Pareto shape estimates for robust PCR (a) and robust pPCR (b). Values less than 0.5 show good model performance and values over 1.0 show poor model performance.}
\label{fig:fortlooplots}
\end{figure}


```{r loo_fort2, echo=FALSE, message=FALSE, include=FALSE, warning=FALSE, fig.height=6, fig.width=12, eval=TRUE}
## PCR
par(mfrow=c(2, 2), mar=c(3.5, 3.5, 1.75, 1) + 0.1, oma=c(0, 0, 0.25, 0.25) + 0.1, mgp=c(2, 1, 0))
plot_k(loo_pca_lasso_fort$pareto_k, main="PCR reconstruction", ylim=c(0, 4.0),
       cex.axis=1.5, cex.main=1.5, cex.lab=1.5)
mtext("(a)",side=3,line=-1.9,adj=c(0.1),cex=2,col="black")
plot_k(loo_pca_lasso_outlier$pareto_k, main="PCR outlier removed", ylim=c(0, 4.0),
       cex.axis=1.5, cex.main=1.5, cex.lab=1.5)
mtext("(b)",side=3,line=-1.9,adj=c(0.1),cex=2,col="black")
plot_k(loo_tpca_lasso_outlier$pareto_k, main="robust PCR reconstruction", ylim=c(0, 4.0),
       cex.axis=1.5, cex.main=1.5, cex.lab=1.5)
mtext("(c)",side=3,line=-1.9,adj=c(0.1),cex=2,col="black")
plot_k(loo_tppca_lasso_outlier$pareto_k, main="robust pPCR reconstruction", ylim=c(0, 4.0),
       cex.axis=1.5, cex.main=1.5, cex.lab=1.5)
mtext("(d)",side=3,line=-1.9,adj=c(0.1),cex=2,col="black")
```


```{r loo_fort3, echo=FALSE, message=FALSE, include=FALSE, warning=FALSE, fig.height=6, fig.width=12, eval=TRUE}
## PCR
# par(mfrow=c(2, 4), mar=c(3.5, 3.5, 1.75, 1) + 0.1, oma=c(0, 0, 0.25, 0.25) + 0.1, mgp=c(2, 1, 0))
layout(matrix(1:8, 2, 4, byrow=FALSE))
par(mar=c(3.5, 3.5, 1.75, 0.25) + 0.1, oma=c(0, 0, 0.25, 0.25) + 0.1, mgp=c(2, 1, 0))

plot_k(loo_pca_lasso_fort$pareto_k, main="PCR", ylim=c(0, 4.0),
       cex.axis=1.5, cex.main=1.5, cex.lab=1.5)
mtext("(a)",side=3,line=-1.9,adj=c(0.1),cex=2,col="black")
plot_k(loo_pca_lasso_outlier$pareto_k, main="PCR outlier removed", ylim=c(0, 4.0),
       cex.axis=1.5, cex.main=1.5, cex.lab=1.5)
mtext("(e)",side=3,line=-1.9,adj=c(0.1),cex=2,col="black")

plot_k(loo_ppca_lasso_fort$pareto_k, main="pPCR", ylim=c(0, 4.0),
       cex.axis=1.5, cex.main=1.5, cex.lab=1.5)
mtext("(b)",side=3,line=-1.9,adj=c(0.1),cex=2,col="black")
plot_k(loo_ppca_lasso_outlier$pareto_k, main="pPCR outlier removed", ylim=c(0, 4.0),
       cex.axis=1.5, cex.main=1.5, cex.lab=1.5)
mtext("(f)",side=3,line=-1.9,adj=c(0.1),cex=2,col="black")

plot_k(loo_tpca_lasso_fort$pareto_k, main="robust PCR", ylim=c(0, 4.0),
       cex.axis=1.5, cex.main=1.5, cex.lab=1.5)
mtext("(c)",side=3,line=-1.9,adj=c(0.1),cex=2,col="black")
plot_k(loo_tpca_lasso_outlier$pareto_k, main="robust PCR outlier removed", ylim=c(0, 4.0),
       cex.axis=1.5, cex.main=1.5, cex.lab=1.5)
mtext("(g)",side=3,line=-1.9,adj=c(0.1),cex=2,col="black")
plot_k(loo_tppca_lasso_fort$pareto_k, main="robust pPCR", ylim=c(0, 4.0),
       cex.axis=1.5, cex.main=1.5, cex.lab=1.5)
mtext("(d)",side=3,line=-1.9,adj=c(0.1),cex=2,col="black")
plot_k(loo_tppca_lasso_outlier$pareto_k, main="robust pPCR outlier removed", ylim=c(0, 4.0),
       cex.axis=1.5, cex.main=1.5, cex.lab=1.5)
mtext("(h)",side=3,line=-1.9,adj=c(0.1),cex=2,col="black")
```


```{r loo-jsm, echo=FALSE, message=FALSE, include=FALSE, warning=FALSE, fig.height=6, fig.width=12, eval=TRUE}
## PCR
# par(mfrow=c(2, 4), mar=c(3.5, 3.5, 1.75, 1) + 0.1, oma=c(0, 0, 0.25, 0.25) + 0.1, mgp=c(2, 1, 0))
png("~/Google Drive/Meetings/2017/JSM/ASCMO/loo.png", 
    width=480*6, height=480*4, res=400)
layout(matrix(1:4, 2, 2, byrow=FALSE))
par(mar=c(3.5, 3.5, 1.75, 0.25) + 0.1, oma=c(0, 0, 0.25, 0.25) + 0.1, mgp=c(2, 1, 0))

plot_k(loo_pca_lasso_outlier$pareto_k, main="PCR", ylim=c(0, 4.0),
       cex.axis=1.5, cex.main=1.5, cex.lab=1.5)
plot_k(loo_ppca_lasso_outlier$pareto_k, main="pPCR", ylim=c(0, 4.0),
       cex.axis=1.5, cex.main=1.5, cex.lab=1.5)
plot_k(loo_tpca_lasso_outlier$pareto_k, main="robust PCR", ylim=c(0, 4.0),
       cex.axis=1.5, cex.main=1.5, cex.lab=1.5)
plot_k(loo_tppca_lasso_outlier$pareto_k, main="robust pPCR", ylim=c(0, 4.0),
       cex.axis=1.5, cex.main=1.5, cex.lab=1.5)
dev.off()
```
...
```{r fortPlotstPCA-jsm, echo=TRUE, eval=TRUE, include=FALSE, fig.height=6, fig.width=12, eval=TRUE}
library(RColorBrewer)
fort.raster.tmp <- fort.raster
fort.raster.sd <- fort.raster
min.fort <- rep(NA, 4)
...
png("~/Google Drive/Meetings/2017/JSM/ASCMO/robust-pca-mean.png", 
    width=480*6, height=480*4, res=400)
layout(matrix(c(1, 1, 1, 2, 2, 2, 5,
                1, 1, 1, 2, 2, 2, 5,
                1, 1, 1, 2, 2, 2, 5,
...
png("~/Google Drive/Meetings/2017/JSM/ASCMO/robust-pca-sd.png", 
    width=480*6, height=480*4, res=400)
layout(matrix(c(1, 1, 1, 2, 2, 2, 5,
                1, 1, 1, 2, 2, 2, 5,
                1, 1, 1, 2, 2, 2, 5,
...
for(i in subset[1:2]){
  image(fort.raster.sd[[i]], main = paste(i + 1819),
        col=rev(colorRampPalette(brewer.pal(8, "YlGn"))(16)),
        # cm.colors(64),
        zlim = c(min(min.fort.sd), max(max.fort.sd)),
        ylab = '', xlab='', yaxt="n", xaxt="n", cex.main=2.5)
  map(database = 'state', add = TRUE, col = 'black')
...
image.scale(fort.raster.sd[[i]], col=rev(colorRampPalette(brewer.pal(8, "YlGn"))(16)),
            # cm.colors(64),
            zlim = c(min(min.fort.sd), max(max.fort.sd)), horiz=FALSE)
dev.off()
...
png("~/Google Drive/Meetings/2017/JSM/ASCMO/robust-ppca-mean.png", 
    width=480*6, height=480*4, res=400)
layout(matrix(c(1, 1, 1, 2, 2, 2, 5,
                1, 1, 1, 2, 2, 2, 5,
                1, 1, 1, 2, 2, 2, 5,
...
png("~/Google Drive/Meetings/2017/JSM/ASCMO/robust-ppca-sd.png", 
    width=480*6, height=480*4, res=400)
layout(matrix(c(1, 1, 1, 2, 2, 2, 5,
                1, 1, 1, 2, 2, 2, 5,
                1, 1, 1, 2, 2, 2, 5,
...
\begin{figure}
\centering\includegraphics[width=0.95\linewidth]{observer_files/figure-latex/fortPlotstPCA-1.pdf}
...
\begin{figure}
\centering\includegraphics[width=0.95\linewidth]{observer_files/figure-latex/fortPlotsDiff-1.pdf}
...
library(ggplot2)
time.dat <- data.frame(
  Temperature = c(madison_temp[1:75, ], minneapolis_temp[1:75, ], 
  ...
  model = rep(c(rep("robust PCR", 75), rep("robust pPCR", 75)), times=4),
...
png("~/Google Drive/Meetings/2017/JSM/ASCMO/time-series.png", 
    width=480*6, height=480*4, res=400)
ggdraw() +
  draw_plot(grid.arrange(ts1), 0.01, 0.2, 1, .8) +
  draw_plot(grid.arrange(ts2), 0, 0, 0.85, 0.2)
dev.off()
...
grid.text("(a)", x = unit(0.95, "npc"), 
          y = unit(.95, "npc"), gp = gpar(fontsize=20))
grid.text("(b)", x = unit(0.95, "npc"), 
          y = unit(.25, "npc"), gp = gpar(fontsize=20))
...
\begin{figure}
\centering\includegraphics[width=0.95\linewidth]{observer_files/figure-latex/TimeSeries-4.pdf}
...
for(i in 1:t){
  values(fort.raster.tmp[[i]])[ - na.rows] <- y_tilde_mean_tppca_lasso_outlier[, i]
  values(fort.raster.sd[[i]])[ - na.rows] <- y_tilde_sd_tppca_lasso_outlier[, i]
  min.fort[i] <- min(c(y_tilde_mean_tpca_lasso_outlier[, i],
                     y_tilde_mean_tppca_lasso_outlier[, i]))
  max.fort[i] <- max(c(y_tilde_mean_tpca_lasso_outlier[, i],
                     y_tilde_mean_tppca_lasso_outlier[, i]))
  min.fort.sd[i] <- min(c(y_tilde_sd_tpca_lasso_outlier[, i],
                        y_tilde_sd_tppca_lasso_outlier[, i]))
  max.fort.sd[i] <- max(c(y_tilde_sd_tpca_lasso_outlier[, i],
                        y_tilde_sd_tppca_lasso_outlier[, i]))
}
par(mfrow=c(4, 4), mar=c(3, 3, 1.75, 1.5) + 0.1, oma=c(0, 0, 0.25, 0.5) + 0.1, mgp=c(2, 1, 0))
# par(mfrow=c(4, 4), mar=c(3, 3, 1, 1) + 0.1, oma=c(0, 0, 0, 0) + 0.1, mgp=c(2, 1, 0))
